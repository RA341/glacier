<script lang="ts">
    ...
    function getStatusTheme(state: string) {
        ...
        if (state?.toLowerCase().includes('complete')) return {color: 'bg-green-500', icon: CheckCircle2Icon};
        ...
        {:else}
            ...
            <div class="grid grid-cols-1 gap-4">
                {#each activeDownloadsRpc.value?.game ?? [] as download (download.ID)}
                    ...
                    <div
                            class="relative group bg-surface border border-border rounded-3xl p-5 flex items-center gap-6 overflow-hidden hover:border-frost-500/30 transition-all shadow-sm"
                            in:fly={{ y: 10, duration: 300 }}
                    >

                        <!-- Game Image (Right Side) -->
                        <div class="w-20 h-28 shrink-0 rounded-xl border border-border bg-panel overflow-hidden shadow-inner">
                            {#if download.Meta?.ThumbnailURL}
                                <img src={download.Meta.ThumbnailURL} alt="" class="w-full h-full object-cover"/>
                            {:else}
                        ...
                        <div class="flex-1 space-y-4">
                            ...
                            <div class="flex items-center justify-between">
                                <h3 class="font-bold text-lg text-foreground truncate max-w-md">
                                    {download.Meta?.Name || 'Unknown Title'}
                                </h3>
                                <span class="px-3 py-1 rounded-full bg-panel border border-border text-[10px] font-bold uppercase tracking-wider text-muted flex items-center gap-2">
                                    <span class="w-1.5 h-1.5 rounded-full {theme.color} animate-pulse"></span>
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between text-xs font-mono text-muted mb-1">
                                    <span>{download.DownloadState?.Progress || 'Initializing...'}</span>
                                ...
                                <div class="h-2 w-full bg-panel rounded-full overflow-hidden border border-border/50">
                                    <div
                                            class="h-full {theme.color} transition-all duration-500 ease-out"
